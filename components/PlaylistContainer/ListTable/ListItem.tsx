...
export default function ListItem({ song, index }: ListItemProps) {
    ...
    return (
        <div key={song.song_id} className="flex items-center justify-between h-14 rounded-md hover:bg-hover-gray-background text-gray-text hover:!text-white">
            <div className={`text-center w-[5%]` + (playerState.song?.song_id === song.song_id ? " text-green-500" : "")}>{index + 1}</div>
            <div className="text-left w-2/5 flex space-x-3 h-10">
            ...
            <Link href={`/app/album/${song.album_id}`} className="text-left w-1/5 truncate overflow-hidden hover:underline">{song.album.name}</Link>
            <div className="text-left w-1/6 truncate overflow-hidden">{new Date(song.playlist_song[0].created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
            <div className="text-center w-1/12 truncate overflow-hidden">{song.duration_ms}</div>
            <div className="w-[5%]">
